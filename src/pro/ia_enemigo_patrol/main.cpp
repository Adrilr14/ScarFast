...
int main() {
  ...
  //Creamos enemy
  std::vector<sf::Vector2f> patrol; //Patrulla para la ia del enemigo
  patrol.push_back(sf::Vector2f(480.f, 360.f));
  patrol.push_back(sf::Vector2f(300.f, 360.f));
  patrol.push_back(sf::Vector2f(300.f, 290.f));
  patrol.push_back(sf::Vector2f(480.f, 290.f));
  sf::Sprite* spriteEnemy = createSprite("sprites.png", sf::Vector2f(480.f, 360.f), 75, 75, 2, 2);
  int idEnemy = 1;
  int hpEnemy = 10;
  int damageEnemy = 10;
  float velocityEnemy = 0.01f;
  IA *iaPatrolEnemy = new IA(patrol);
  Enemy *enemy = new Enemy(idEnemy, hpEnemy, damageEnemy, velocityEnemy, spriteEnemy, iaPatrolEnemy);
  ...
  while (window->isOpen()) {
    ...
    {
      disparoBola->mover();
      if (col->collidesWith(disparoBola->getSprite(), enemy->getSprite()))
      {
        if(enemy->getHP() > 0){
          int newHP = enemy->getHP() - disparoBola->getDamage();
          enemy->setHP(newHP);
          disparoBola = nullptr;
        }
      }
